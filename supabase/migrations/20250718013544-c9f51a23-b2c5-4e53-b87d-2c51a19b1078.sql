-- Create some sample campaigns linked to existing personas (without organization_id)
INSERT INTO public.campaigns (user_id, persona_id, title, description, campaign_type, status, budget, start_date, end_date, objectives, target_metrics, channels, creative_assets)
SELECT 
  p.user_id,
  p.id as persona_id,
  CASE 
